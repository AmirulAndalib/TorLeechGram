...
class TorLeechGramDB(DataBaseHandle):
    def __init__(self,dburl=None):
        ...
        settings_schema = """CREATE TABLE IF NOT EXISTS tlg_config (
    ...
    def set_variable(self,var_name,var_value,update_blob=False,blob_val=None):
        ...
        sql = "SELECT * FROM tlg_config WHERE var_name=%s"
        cur = self.scur()
        
        cur.execute(sql,(var_name,))
        if cur.rowcount > 0:
            if not update_blob:
                sql = "UPDATE tlg_config SET var_value=%s , vtype=%s WHERE var_name=%s"
            else:
                sql = "UPDATE tlg_config SET blob_val=%s , vtype=%s WHERE var_name=%s"
                var_value = blob_val

            cur.execute(sql,(var_value,vtype,var_name))
        else:
            if not update_blob:
                sql = "INSERT INTO tlg_config(var_name,var_value,date_changed,vtype) VALUES(%s,%s,%s,%s)"
            else:
                sql = "INSERT INTO tlg_config(var_name,blob_val,date_changed,vtype) VALUES(%s,%s,%s,%s)"
                var_value = blob_val
...
class TlgUpload(DataBaseHandle):
    ...
    def __init__(self,dburl=None):
        ...
        uploads_schema = """CREATE TABLE IF NOT EXISTS tlg_uploads (
            id SERIAL PRIMARY KEY NOT NULL,
    ...
    def register_upload(self,chat_id,mes_id,is_batch=False):
        ...
        cur = self.scur()
        sql = "SELECT * FROM tlg_uploads WHERE chat_id=%s and message_id=%s"
        
        cur.execute(sql,(chat_id,mes_id))

        if cur.rowcount > 0:
            row = cur.fetchone()
            sql = "DELETE FROM tlg_uploads WHERE id=%s"
            cur.execute(sql,(row[0],))

        sql = "INSERT INTO tlg_uploads(chat_id,message_id,is_batch) VALUES(%s,%s,%s)"
    ...
    def cancel_download(self,chat_id,mes_id):
        ...
        if cur.rowcount == 0:
            ...
            sql = "UPDATE tlg_uploads SET cancel=TRUE WHERE chat_id=%s and message_id=%s"
            cur.execute(sql,(chat_id,mes_id))
            self.ccur(cur)
            return True
    ...
    def deregister_upload(self,chat_id,mes_id):
        ...
        try:
            ...
            pass
        sql = "DELETE FROM tlg_uploads WHERE chat_id=%s and message_id=%s"
        cur = self.scur()
        cur.execute(sql,(chat_id,mes_id))
        self.ccur(cur)
...
class TlgTorrents(DataBaseHandle):
    def __init__(self,dburl=None):
        if dburl is None:
            dburl = os.environ.get("DB_URI",None)
            if dburl is None:
        ...
        super().__init__(dburl)
        cur = self.scur()
        table = """CREATE TABLE IF NOT EXISTS tlg_torrents(
            id SERIAL PRIMARY KEY NOT NULL,
    ...
    def add_torrent(self,hash_id,passw):
        sql = "SELECT * FROM tlg_torrents WHERE hash_id=%s"
        cur = self.scur()
        cur.execute(sql,(hash_id,))
        if cur.rowcount > 0:
            sql = "UPDATE tlg_torrents SET passw=%s WHERE hash_id=%s"
            cur.execute(sql,(passw,hash_id))
        else:
            sql = "INSERT INTO tlg_torrents(hash_id,passw) VALUES(%s,%s)"
    ...
    def disable_torrent(self,hash_id):
        ...
        cur.execute(sql,(hash_id,))
        if cur.rowcount > 0:
            sql = "UPDATE tlg_torrents SET enab=false WHERE hash_id=%s"
            cur.execute(sql,(hash_id,))
...
class UserDB(DataBaseHandle):
    ...
    def __init__(self,dburl=None):
        ...
        super().__init__(dburl)
        cur = self.scur()
        table = """CREATE TABLE IF NOT EXISTS tlg_users(
            id SERIAL PRIMARY KEY NOT NULL,
    ...
    def get_var(self, var, user_id):
        user_id = str(user_id)
        sql = "SELECT * FROM tlg_users WHERE user_id=%s"
        # search the cache
    ...
    def set_var(self, var, value, user_id):
        ...
        cur.execute(sql, (user_id,))
        if cur.rowcount > 0:
            insql = "UPDATE tlg_users SET json_data = %s where user_id=%s"
            cur.execute(insql, ( json.dumps(self.shared_users.get(user_id)), user_id))

        else:
            insql = "INSERT INTO tlg_users(user_id, json_data) VALUES(%s, %s)"
    ...
    def set_rclone(self, rclonefile, user_id):
        ...
        cur.execute(sql, (user_id,))
        if cur.rowcount > 0:
            insql = "UPDATE tlg_users SET rclone_file=%s WHERE user_id=%s"
            cur.execute(insql, (rclonefile, user_id))

        else:
            insql = "INSERT INTO tlg_users(user_id, json_data, rclone_file) VALUES(%s, '{}', %s)"
    ...
    def set_thumbnail(self, thumbnail, user_id):
        ...
        cur.execute(sql, (user_id,))
        if cur.rowcount > 0:
            insql = "UPDATE tlg_users SET thumbnail=%s WHERE user_id=%s"
            cur.execute(insql, (thumbnail, user_id))

        else:
            insql = "INSERT INTO tlg_users(user_id, json_data, thumbnail) VALUES(%s, '{}', %s)"
