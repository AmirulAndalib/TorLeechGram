...
from . import nodes
from .database_handle import TlgTorrents
...
<body>
<h1>TorLeechGram : <a href="#">Github</a></h1>
...
@routes.get('/torlg/files/{hash_id}')
async def list_torrent_contents(request):
    ...
    if not "pin_code" in gets.keys():
        rend_page = code_page.replace("{form_url}",f"/torlg/files/{torr}")
    ...
    rend_page = rend_page.replace("{form_url}",f"/torlg/files/{torr}?pin_code={pincode}")
    client.auth_log_out()
    return web.Response(text=rend_page,content_type='text/html')
...
@routes.post('/torlg/files/{hash_id}')
...
async def homepage(request):
    return web.Response(text="<h1>See TorTookit <a href=\"#\">@GitHub</a> By Priiiyo</h1>",content_type="text/html")
  ...
  async def middleware_handler(request):
      try:
          ...
          if response.status == 404:
              return web.Response(text="<h1>404: Page not found</h2><br><h3>TorLeechGram</h3>",content_type="text/html")
          return response
